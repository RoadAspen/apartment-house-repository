...
export const getAcceptanceStats = () => {
  return request({
    url: '/api/stats/acceptance',
    method: 'get'
  });
};

// 获取每日请求次数
export const getDailyRequests = () => {
  return request({
    url: '/api/stats/daily_requests',
    method: 'get'
  });
};

// 获取用户调用接口次数 Top10
export const getUserTop10 = () => {
  return request({
    url: '/api/stats/user_top10',
    method: 'get'
  });
};
